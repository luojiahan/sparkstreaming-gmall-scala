package com.cuiyf41.dws

import java.lang
import java.util.Properties
import com.alibaba.fastjson.JSON
import com.alibaba.fastjson.serializer.SerializeConfig
import com.cuiyf41.bean.{OrderDetail, OrderInfo, OrderWide}
import com.cuiyf41.util.{MyKafkaSink, MyKafkaUtil, MyPropertiesUtil, MyRedisUtil, OffsetManagerUtil}
...
object OrderWideApp {
  ...
  private val clickhosueUrl: String = prop.getProperty("jdbc.clickhouse.url")
  def main(args: Array[String]): Unit = {
    ...
    if(orderInfoOffsetMap!= null && orderInfoOffsetMap.size > 0){
      orderInfoRecordDStream = MyKafkaUtil.getKafkaDStream(ssc,orderInfoTopic,orderInfoGroupId,orderInfoOffsetMap)
    }else{
      orderInfoRecordDStream = MyKafkaUtil.getKafkaDStream(ssc,orderInfoTopic,orderInfoGroupId)
    }
    ...
    if(orderDetailOffsetMap!= null && orderDetailOffsetMap.size > 0){
      orderDetailRecordDStream = MyKafkaUtil.getKafkaDStream(ssc,orderDetailTopic,orderDetailGroupId,orderDetailOffsetMap)
    }else{
      orderDetailRecordDStream = MyKafkaUtil.getKafkaDStream(ssc,orderDetailTopic,orderDetailGroupId)
    }
    ...
    orderWideSplitDStream.foreachRDD{
      rdd=>{
        ...
        df.write.mode(SaveMode.Append)
          ...
          .jdbc(clickhosueUrl,"t_order_wide",new Properties())
