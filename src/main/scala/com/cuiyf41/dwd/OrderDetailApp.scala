package com.cuiyf41.dwd

import com.alibaba.fastjson.serializer.SerializeConfig
import com.alibaba.fastjson.{JSON, JSONObject}
import com.cuiyf41.bean.{OrderDetail, SkuInfo}
import com.cuiyf41.util.{MyKafkaSink, MyKafkaUtil, OffsetManagerUtil, PhoenixUtil}
...
object OrderDetailApp {
  def main(args: Array[String]): Unit = {
    ...
    if (offsetMapForKafka != null && offsetMapForKafka.size > 0) {
      recordInputDstream = MyKafkaUtil.getKafkaDStream(ssc, topic, groupId, offsetMapForKafka)
    } else {
      recordInputDstream = MyKafkaUtil.getKafkaDStream(ssc, topic, groupId)
    }
